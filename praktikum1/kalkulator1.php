            ...
            while ($this->pos < count($this->tokens) && !$this->error) {
                ...
                if ($op === '*' || $op === '/' || $op === '%') {
                    ...
                    if ($op === '*') {
                        ...
                        if ($right == 0) {
                            $this->error = 'Modulo dengan nol tidak diperbolehkan.';
                            return 0;
                        }
                        $val = fmod($val, $right);
                    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ekspresi === '' && isset($_POST['a']) && isset($_POST['b'])) {
        $a_raw = trim($_POST['a'] ?? '');
        $b_raw = trim($_POST['b'] ?? '');
    ...
    if ($ekspresi === '') {
        ...
        if ($parser->error) {
            ...
            $hasil = $res;
            // Sinkronisasi variabel $a, $b, $operator jika ekspresi adalah operasi biner sederhana (misal: -1 + 1)
    ...
    <script>
        const calculator = {
            ...
            appendOp(op) {
                ...
                // Jika karakter terakhir sudah operator, dan yang ditekan adalah minus (bisa sebagai tanda negatif)
                ...
                // Jika karakter terakhir operator dan ingin diganti operator lain selain minus
            ...
            toggleSign() {
                ...
                // Cek angka terakhir dalam ekspresi untuk di-toggle tandanya
